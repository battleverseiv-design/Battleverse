...
local function runReloadAnimation(context)
    local track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
    if (context:getAmmoCount() >= 6) then
        context:runAnimation("reload_up", track, false, PLAY_ONCE_STOP, 0.2)
    else
        context:runAnimation("reload_up_and_down", track, false, PLAY_ONCE_STOP, 0.2)
    end
end
...
local function runInspectAnimation(context)
    ...
    if (context:getAmmoCount() >= 6) then
        ...
        context:runAnimation("inspect_down", track, false, PLAY_ONCE_STOP, 0.2)
    end
end
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        local ammoCount = context:getAmmoCount()
        if (ammoCount > 6) then
            context:runAnimation("shoot_up", track, true, PLAY_ONCE_STOP, 0)
        else
            context:runAnimation("shoot_down", track, true, PLAY_ONCE_STOP, 0)
        end
    end
    return nil
end
