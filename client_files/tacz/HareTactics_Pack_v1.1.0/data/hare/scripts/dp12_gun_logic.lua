...
function M.start_reload(api)
    ...
    local cache = {
        reloaded_count = 0,
        needed_count = api:getNeededAmmoAmount() % 2 ~= 0 and api:getNeededAmmoAmount() - 1 or api:getNeededAmmoAmount(),
...
function M.tick_reload(api)
    ...
    if (reloaded_count >= 0) then
        local base_time = (reloaded_count/2) * loop + loop_feed
        ...
        while (base_time < reload_time) do
            ...
            api:putAmmoInMagazine(api:isReloadingNeedConsumeAmmo() and api:consumeAmmoFromPlayer(2) or 2)
        end
    end
    -- Write back cache
    if (reloaded_count >= cache.needed_count) then
