...
function idle_state.transition(this, context, input)
    ...
    if (input == INPUT_RELOAD) then
        if (isNoAmmo(context)) then
            if(context:getFireMode() == BURST) then
                context:runAnimation("reload_empty_semi", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
            else
            ...
            if (context:getFireMode() == BURST) then
                context:runAnimation("reload_tactical_semi", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
            else
