...
function M.start_reload(api)
    ...
    if (api:getAmmoAmount() == 0 and api:hasAmmoInBarrel()) then
        cache.reload_type = "RELOAD_01"
    elseif (not api:hasAmmoInBarrel()) then
        cache.reload_type = "RELOAD_EMPTY"
    else
        cache.reload_type = "NULL"
    end
...
local function getReloadTimingFromParam(param)
    ...
    local reload_01_cooldown = param.reload_01_cooldown * 1000
    local reload_01_feed = param.reload_01_feed * 1000
    -- 检查是否任意时间为空值
    if (intro_empty == nil or intro == nil or loop == nil or loop_2 == nil or ending == nil or intro_empty_feed == nil or loop_feed == nil or loop_feed_2 == nil or reload_empty_cooldown == nil or reload_empty_feed == nil or reload_01_cooldown == nil or reload_01_feed == nil) then
        return nil
    end
    -- 依次返回时间
    return intro_empty, intro, loop, loop_2, ending, intro_empty_feed, loop_feed, loop_feed_2, reload_empty_cooldown, reload_empty_feed, reload_01_cooldown, reload_01_feed
end

function M.tick_reload(api)
    -- 从枪的 data 文件里获取所有脚本需要的参数值
    local param = api:getScriptParams();
    local intro_empty, intro, loop, loop_2, ending, intro_empty_feed, loop_feed, loop_feed_2, reload_empty_cooldown, reload_empty_feed, reload_01_cooldown, reload_01_feed = getReloadTimingFromParam(param)
    -- 象征性的检查一下时间有没有空值          检查你妈啊上面都他妈查过了你几把再查一遍你他妈有病吧
    ...
    if (cache.reload_type == "RELOAD_EMPTY") then
        ...
        elseif (reload_time > reload_empty_cooldown) then
            return NOT_RELOADING, -1
        end
    elseif (cache.reload_type == "RELOAD_01") then
        if (reload_time < reload_01_feed) then
            return EMPTY_RELOAD_FEEDING, reload_01_feed - reload_time
        elseif (reload_time >= reload_01_feed and reload_time <= reload_01_cooldown) then
            if (not cache.reloaded) then
                if (api:isReloadingNeedConsumeAmmo()) then
            ...
            return EMPTY_RELOAD_FINISHING, reload_01_cooldown - reload_time
        elseif (reload_time > reload_01_cooldown) then
            return NOT_RELOADING, -1
        end
    else
