...
local default = require("ccrp_underbarrel_standard")
...
local function runReloadAnimation(context)
    ...
    if (isNoAmmo(context)) then
        ...
        elseif (ext == 2) then
            context:runAnimation("reload_empty_xmag", track, false, PLAY_ONCE_STOP, 0.2)
        elseif (ext == 3) then
            context:runAnimation("reload_empty_drum_rpk", track, false, PLAY_ONCE_STOP, 0.2)
        else
            context:runAnimation("reload_empty", track, false, PLAY_ONCE_STOP, 0.2)
        end
    else
        ...
        elseif (ext == 2) then
            context:runAnimation("reload_tactical_xmag", track, false, PLAY_ONCE_STOP, 0.2)
        elseif (ext == 3) then
            context:runAnimation("reload_tactical_drum_rpk", track, false, PLAY_ONCE_STOP, 0.2)
        else
            context:runAnimation("reload_tactical", track, false, PLAY_ONCE_STOP, 0.2)
        end
    end
...
local function runInspectAnimation(context)
    ...
    if (isNoAmmo(context)) then
        -- "ext"表示扩容插件等级,"0"代表不装扩容插件
        if (ext == 0) then
            context:runAnimation("inspect_empty_mwii", track, false, PLAY_ONCE_STOP, 0.2)
        elseif (ext == 1) then
            context:runAnimation("inspect_empty_mwii", track, false, PLAY_ONCE_STOP, 0.2)
        elseif (ext == 2) then
            context:runAnimation("inspect_empty_mwii", track, false, PLAY_ONCE_STOP, 0.2)
        ...
        if (ext == 0) then
            context:runAnimation("inspect_mwii", track, false, PLAY_ONCE_STOP, 0.2)
        elseif (ext == 1) then
            context:runAnimation("inspect_mwii", track, false, PLAY_ONCE_STOP, 0.2)
        elseif (ext == 2) then
            context:runAnimation("inspect_mwii", track, false, PLAY_ONCE_STOP, 0.2)
...
function fire_mode_state.semi.transition(this, context,input)
    if(input == this.INPUT_MODE_AUTO)then
        local transform_track = context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)
        if (context:isStopped(transform_track)) then
