...
function bolt_caught_states.normal.update(this, context)
    ...
    if (isNoAmmo(context) and not UsingUnderbarrel) then
        if (context:getCurrentTimestamp() - ChangeTime >= 100) then
...
function bolt_caught_states.bolt_caught.update(this, context)
    ...
    if (not isNoAmmo(context) and not UsingUnderbarrel) then
        if (context:getCurrentTimestamp() - ChangeTime >= 100) then
...
function main_track_states.start.transition(this, context, input)
    ...
    if (input == INPUT_DRAW) then
        ...
        local nbtString = tostring(context:getNbtAccessor():getCompound("Extras"))
        local startIdx, endIdx = string.find(nbtString, "UsingUnderBarrel:%s*(%d)b")
        if (pcall(function()return tonumber(string.sub(nbtString, startIdx + 18, endIdx - 1)) == 1 end)) then
...
function main_track_states.idle.transition(this, context, input)
    if (input == "sub_transform") then
        UsingUnderbarrel = true
        ChangeTime = context:getCurrentTimestamp()
