...
function idle_state.transition(this, context, input)
    ...
    if (input == INPUT_BOLT) then
        if (isSpecial()) then
            context:runAnimation("bolt2", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_STOP, 0.2)
        else
...
function reload_state.entry(this, context)
    ...
    if (isNoAmmo) then
        ...
        context:runAnimation("reload_empty_start", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
    else
        state.timestamp = -1
