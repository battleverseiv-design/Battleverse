...
function base_track_state.entry(this, context)
    ...
    main_track_states.static_idle = 0
    context:runAnimation("static_idle", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, PLAY_ONCE_HOLD, 0.2)
end

function base_track_state.update(this, context)
    local track = context:getTrack(STATIC_TRACK_LINE, BASE_TRACK)
    if (context:isHolding(track)) then
        if (main_track_states.static_idle == 0) then
            context:runAnimation("static_idle_1", track, false, PLAY_ONCE_HOLD, 0.2)
            main_track_states.static_idle = 1
        else
            context:runAnimation("static_idle", track, false, PLAY_ONCE_HOLD, 0.2)
            main_track_states.static_idle = 0
        end
    end
end
