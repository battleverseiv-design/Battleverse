...
local function runReloadAnimation(context)
    ...
    if (isNoAmmo(context)) then
        if (context:isCrawl() == true) then
            context:runAnimation("reload_empty_on", track, false, PLAY_ONCE_STOP, 0.2)
        else
...
local function runInspectAnimation(context)
    ...
    if (isNoAmmo(context)) then
        if (context:isCrawl() == true) then
            context:runAnimation("inspect_empty_on", track, false, PLAY_ONCE_STOP, 0.2)
        else
            context:runAnimation("inspect_empty", track, false, PLAY_ONCE_STOP, 0.2)
        end
    else
        if (context:isCrawl() == true) then
            context:runAnimation("inspect_on", track, false, PLAY_ONCE_STOP, 0.2)
        else
...
function base_track_state.normal.update(this, context)
    if (context:isCrawl() == true) then
        context:trigger(this.INPUT_BIPOD_UNFOLD)
    end
end

function base_track_state.normal.transition(this, context, input)
    if (input == this.INPUT_BIPOD_UNFOLD) then
        context:runAnimation("bipod_on", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, PLAY_ONCE_HOLD, 0)
        return this.base_track_state.crawl
    end
end

function base_track_state.crawl.entry(this, context)
    local track = context:getTrack(STATIC_TRACK_LINE, BASE_TRACK)
    if (context:isStopped(track) or context:isHolding(track)) then
        context:runAnimation("static_idle_on", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, PLAY_ONCE_HOLD, 0)
    end
end

function base_track_state.crawl.update(this, context)
    if (context:isCrawl() == false) then
        context:trigger(this.INPUT_BIPOD_FOLD)
    end
end

function base_track_state.crawl.transition(this, context, input)
    if (input == this.INPUT_BIPOD_FOLD) then
        context:runAnimation("bipod_off", context:getTrack(STATIC_TRACK_LINE, BASE_TRACK), false, PLAY_ONCE_HOLD, 0)
...
function main_track_states.inspect.entry(this, context)
    if (context:isCrawl() == true) then
        main_track_states.crawl = 1
    else
        main_track_states.crawl = 0
    end
...
function main_track_states.inspect.update(this, context)
    if (context:isCrawl() == true) then
        main_track_states.crawl_update = 1
    else
        main_track_states.crawl_update = 0
    end
    if (main_track_states.crawl_update ~= main_track_states.crawl) then
        context:stopAnimation(context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK))
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        if (context:isCrawl() == true) then
            context:runAnimation("shoot_on", track, true, PLAY_ONCE_STOP, 0)
        else
...
local M = {
    ...
    INPUT_BIPOD_UNFOLD = "bipod_unfold",
    INPUT_BIPOD_FOLD = "bipod_fold"
}
