...
function idle_state.transition(this, context, input)
    if (input == INPUT_RELOAD) then
        if (isNoAmmo(context)) then
            ...
            reload_state.reload_type = "TACTICAL"
        end
        reload_state.empty_animation_count = 1
...
function reload_state.entry(this, context)
    ...
    if (reload_state.reload_type == "EMPTY") then
        context:runAnimation("reload_empty_into", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
    elseif (reload_state.reload_type == "TACTICAL") then
        context:runAnimation("reload_tactical_into", context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK), false, PLAY_ONCE_HOLD, 0.2)
    end
...
function reload_state.update(this, context)
    ...
    if (state.loaded_ammo > state.need_ammo or not context:hasAmmoToConsume()) then
        ...
        if (context:isHolding(track)) then
            if (reload_state.reload_type == "EMPTY") then
                local animationName = "reload_empty_loop" .. tostring(reload_state.empty_animation_count)
                context:runAnimation(animationName, track, false, PLAY_ONCE_HOLD, 0)
                reload_state.empty_animation_count = reload_state.empty_animation_count + 1
            elseif (reload_state.reload_type == "TACTICAL") then
